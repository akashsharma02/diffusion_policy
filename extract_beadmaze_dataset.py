...
PATH_OUPUT_ZARR = f"./data/bead_maze/"
...
def bag2dataset(bag_id):
    ...
    for i in range(min_count):
        ...
        dataset["timestamp"].append(msg.header.stamp.sec + msg.header.stamp.nanosec / 1e9)
    ...
    for i in range(min_count):
        ...
        dataset["digit_thumb"].append(numpy_to_binary(img))

        msg = sync_msg_data["/digit_index/compressed"][i]
        img = img_msg_to_array(msg)
        img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB).astype("uint8")
        dataset["digit_index"].append(numpy_to_binary(img))
...
def convert2zarr():
    ...
    buffer = ReplayBuffer.create_from_path(
