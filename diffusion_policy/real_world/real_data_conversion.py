...
def real_data_to_replay_buffer(
    ...
    if image_keys is not None:
        ...
        episode_video_paths = sorted(
            episode_video_dir.glob("*.mp4"), key=lambda x: int(x.stem)
        )
    ...
    print(f"n_steps: {n_steps}, n_cameras: {n_cameras}, dt: {dt}")
    print(f"episode_starts: {episode_starts}")
    print(f"episode_lengths: {episode_lengths}")
    print(f"num_episodes: {len(episode_lengths)}")
    with tqdm(
        total=n_steps * n_cameras, desc="Loading image data", mininterval=1.0
    ) as pbar:
        # one chunk per thread, therefore no synchronization needed
        with concurrent.futures.ThreadPoolExecutor(
            max_workers=n_encoding_threads
        ) as executor:
            futures = set()
            for episode_idx, episode_length in enumerate(episode_lengths):
                ...
                for i in camera_idxs - this_camera_idxs:
                    ...
                    if image_keys is not None:
                        raise RuntimeError(
                            f"Missing camera {i} at episode {episode_idx}"
                        )
